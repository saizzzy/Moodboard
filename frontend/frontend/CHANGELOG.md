...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest "moodboard-react" -- --template=react  --no-rolldown --no-immediate --eslint`.
- Обновление `vite.config.js` с портом.
- Создание файла проекта (`moodboard-react.esproj`).
